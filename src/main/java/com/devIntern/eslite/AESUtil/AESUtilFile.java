...
public class AESUtilFile {
    ...
    /**
     * Encrypts binary data with password and returns Base64(salt || iv || ciphertext) as bytes.
     * Use this for arbitrary files to avoid corrupting binary data via String conversions.
     */
    public static byte[] encrypt(byte[] plaintext, String password) throws Exception {
        ...
        bb.put(iv);
        bb.put(ciphertext);
        return Base64.getEncoder().encode(bb.array());
    }

    /**
     * Decrypts Base64(salt || iv || ciphertext) provided as bytes and returns the original binary data.
     */
    public static byte[] decrypt(byte[] base64Package, String password) throws Exception {
        byte[] all = Base64.getDecoder().decode(base64Package);
        ByteBuffer bb = ByteBuffer.wrap(all);
